...
public class PartialBuilder extends Builder {
  ...
  protected DecisionForest parseOutput(Job job) throws IOException {
    ...
    int [] npos = new int[Builder.getNumMaps(conf)];
    int [] nneg = new int[Builder.getNumMaps(conf)];
        
    processOutput(job, outputPath, keys, trees, nneg, npos);
    
    return new DecisionForest(Arrays.asList(trees), nneg, npos);
  }
  ...
  protected static void processOutput(JobContext job,
                                      Path outputPath,
                                      TreeID[] keys,
                                      Node[] trees, int [] nneg, int [] npos) throws IOException {
