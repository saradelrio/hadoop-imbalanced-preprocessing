...
public class Step1Mapper extends MapredMapper<LongWritable,Text,TreeID,MapredOutput> {
  ...
  protected void cleanup(Context context) throws IOException, InterruptedException {
    ...
    if(classes_distribution[0] > classes_distribution[1]){
    	npos = classes_distribution[1];
    	nneg = classes_distribution[0];
    }else{
    ...
    for (int treeId = 0; treeId < nbTrees; treeId++) {
      ...
      Node tree = bagging.build(rng,context);
      
      key.set(partition, firstTreeId + treeId);
      
      if (!isNoOutput()) {
        MapredOutput emOut = new MapredOutput(tree, nneg, npos);
