...
public class BuildForest extends Configured implements Tool {
  ...
  public int run(String[] args) throws IOException, ClassNotFoundException, InterruptedException {
    ...
    Group group = gbuilder.withName("Options").withOption(dataOpt).withOption(datasetOpt).withOption(timeOpt).withOption(noCompleteOpt).withOption(minSplitOpt)
        .withOption(minPropOpt).withOption(seedOpt).withOption(partialOpt).withOption(nbtreesOpt)
        .withOption(outputOpt).withOption(helpOpt).withOption(selectionOpt).create();
...
private void buildForest() throws IOException, ClassNotFoundException, InterruptedException {
    ...
    log.info("Número de atributos: "+m);
    ...
    buildForestTime = System.currentTimeMillis();
    
    DecisionForest forest = forestBuilder.build(nbTrees,m);
    
    buildForestTime = System.currentTimeMillis() - buildForestTime;
    
    if(buildTimeIsStored)
      writeToFileBuildTime(DFUtils.elapsedTime(buildForestTime)); 
   
    log.info("Build Time: {}", DFUtils.elapsedTime(buildForestTime));
  ...
  private void writeToInstancesByMapper(int[] nneg, int[] npos) throws IOException {
    FileSystem outFS = outputPath.getFileSystem(getConf());
    FSDataOutputStream ofile = null;
    Path filenamePath = new Path(outputPath, dataName + "_instances_by_mapper").suffix(".txt");
    try    
	  {	        	
      if (ofile == null) {
		...
		returnString.append("Instances by Mapper\n");
		for (int i = 0 ; i < nneg.length ; i++){
		  returnString.append("-------------------------------------------------------").append('\n');
		  returnString.append("nneg\n");
		  returnString.append(
		    		  StringUtils.rightPad(String.valueOf(nneg[i]),5)).append('\n');  
		  returnString.append("npos\n");
		  returnString.append(
	    		  StringUtils.rightPad(String.valueOf(npos[i]),5)).append('\n');   
		  returnString.append("-------------------------------------------------------").append('\n');	  
		}
