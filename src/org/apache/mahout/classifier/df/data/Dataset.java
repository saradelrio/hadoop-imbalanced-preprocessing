...
public class Dataset implements Writable {
   ...
   * @return label's value
   */
  public String getAttString(int attr, double code) {
    // handle the case (prediction is NaN)
    if (Double.isNaN(code)) {
      return "unknown";
    }
    return values[attr][(int) code];
  }
   ...
   *
   * @param attr index of the attribute to check
   * @return true if the attribute is categorical
   */
  public boolean isCategorical(int attr) {
    return attributes[attr].isCategorical();
  }
