...
public class DecisionTreeBuilder implements TreeBuilder {
  ...
  @Override
  public Node build(Random rng, Data data, Context context) {
    if (selected == null) {
    ...
    if (data.getDataset().isNumerical(best.getAttr())) {
      ...
      Node loChild = build(rng, loSubset, context);
      Node hiChild = build(rng, hiSubset, context);
      ...
      for (int index = 0; index < values.length; index++) {
        ...
        children[index] = build(rng, subsets[index], context);
      }
